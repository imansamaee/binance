...
$ # --host=0.0.0.0 - expose the application on all network interfaces (default 127.0.0.1)
$ # --port=5000    - specify the application port (default 5000)  
...
< PROJECT ROOT >
   ...
   |-- config.py                 # Set up the application
   |-- application.py                    # Start the application - WSGI gateway
   |
   |-- ************************************************************************
```
...
< PROJECT ROOT >
   |
   |-- application/
   |    |-- home/                                # Home Blueprint - serve application pages (private area)
   |    |-- base/                                # Base Blueprint - handles the authentication
   |         |-- static/
...
```bash
$ gunicorn --bind 0.0.0.0:8001 run:application
...
```bash
$ waitress-serve --port=8001 run:application
Serving on http://localhost:8001
```
